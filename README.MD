...
Training sessions often lack a structured system for collecting topic-wise feedback from trainees after each class. Without detailed ratings for individual topics, it becomes difficult for trainers to identify which areas need improvement or better explanation. A proper mechanism is required to gather and analyze topic-wise class ratings to improve teaching quality and learning effectiveness.
...
The project includes a root `docker-compose.yml` that starts the complete backend microservices architecture.
